...
let TheClientForm = (props)=>{
    ...
    let [modifiedData, setModifiedData] = React.useState([])
    ...
    React.useEffect(()=>{
        ...
        */
        ...
        if((props.isSubmitable)&&(props.editable===false)){
            console.log("ERROR - isSubmitable is true, but no data can be entered")
        }

        if((props.clientID)&&(props.userData)){
            console.log("Both clientID and userData were sent as props! The component will fetch data and ignore the userData prop!")
        }
        ...
        //just an ID, fetch the data from the DB
            ...
            fetch(`/clients/?filter=id&filterBy=${props.clientID}`,
            {
                method:"GET",
